...
Your working directory is `/workspaces/proofboard-cli/.agents/worker_m2_compliance/`.
Your parent conversation ID is `066f5421-8262-4d3c-a457-bf22bdc942ea`.
Your task is to implement v1.4 spec compliance updates, endpoints review, and release preparation in the Proofboard CLI Go codebase.

### Task Checklist
1. **Version Update**: Update the Version constant in `internal/version/version.go` to `"1.4.0"`, and update any version references in `README.md` and `GEMINI.md` / `AGENTS.md` (Product section) to keep documentation consistent.
