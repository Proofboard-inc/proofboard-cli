...
CREATE_RESP=$(curl -s -X POST -H "Authorization: token $GITHUB_TOKEN" -H "Content-Type: application/json" \
  -d '{"tag_name":"'"$TAG"'","name":"Proofboard CLI v1.8.1","body":"Added secure release signing and verification using ECDSA keys. Removed Phase 6 Handshake. Added local fraud detection. Finalized notification architecture and typography."}' \
