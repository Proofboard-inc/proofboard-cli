...
${STUB_BIN:?set STUB_BIN} > "$STUB_LOG" 2>&1 &
STUB_PID=$!
for _ in $(seq 1 50); do grep -q "STUB READY" "$STUB_LOG" 2>/dev/null && break; sleep 0.1; done
grep -q "STUB READY" "$STUB_LOG" || { echo "notification stub did not start"; cat "$STUB_LOG"; exit 1; }
echo "notification service: ready"

cleanup() { kill $MOCK_PID $STUB_PID 2>/dev/null; rm -rf "$WORK"; }
trap cleanup EXIT
...
echo ""
echo "=== 1. AUTHENTICATE (device code -> poll -> approve) ==="
...
check    "device key id recorded" \
  "$(python3 -c "import json;print(json.load(open('$HOME/.proofboard/device.key')).get('deviceKeyId',''))")" "device-key-e2e"
...
if payloads:
    ...
    ok &= check("deviceKeyId attached", p["deviceKeyId"], "device-key-e2e")
...
echo ""
echo "=== 4. SECOND SYNC CARRIES previousHead ==="
echo 'package confidentialledger // extended' > "$REPO/$SECRET_PATH"
git -C "$REPO" add -A
git -C "$REPO" commit -qm "fix: settle rounding in the confidential ledger"
...
echo ""
echo "=== 5. SIGNATURE AND PRIVACY VERDICT FROM THE BACKEND ==="
...
echo ""
echo "=== 6. BACKGROUND PROJECT DETECTION ==="
...
BEFORE=$(grep -c "^NOTIFY" "$STUB_LOG")
for i in $(seq 1 10); do (cd "$NEWREPO" && "$PB" detect --editor vscode >/dev/null 2>&1); done
sleep 1.5
AFTER=$(grep -c "^NOTIFY" "$STUB_LOG")
check "ten editor terminals raise one prompt" "$((AFTER-BEFORE))" "1"
grep "^NOTIFY" "$STUB_LOG" | tail -1 | sed 's/^/     | /'
...
echo ""
echo "=== 8. BACKGROUND AGENT LIFECYCLE ==="
...
echo ""
echo "=== 9. CALL SEQUENCE OBSERVED BY THE BACKEND ==="
