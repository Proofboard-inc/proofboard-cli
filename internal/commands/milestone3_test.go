...
func TestSyncUnlinkedPromptNeverAsk(t *testing.T) {
	tempHome := t.TempDir()
	repoDir := createTempGitRepo(t)
	...
	if err := cmd.ExecuteContext(ctx); err != nil {
		t.Fatalf("expected exit without error, got: %v", err)
	}
	...
	found := false
	for _, p := range st.SuppressedWorkspaces {
		if p == repoPathAbs {
			found = true
			break
		}
	...
	if !found {
		t.Errorf("expected repository path %q to be in SuppressedWorkspaces, got %v", repoPathAbs, st.SuppressedWorkspaces)
	}
}
